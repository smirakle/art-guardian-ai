...
-- Table for AI agent deployments
...
CREATE TABLE IF NOT EXISTS public.ai_protection_rate_limits (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_ai_monitoring_agents_platform ON public.ai_monitoring_agents(platform_id);
CREATE INDEX IF NOT EXISTS idx_ai_threat_detections_user_id ON public.ai_threat_detections(user_id);
CREATE INDEX IF NOT EXISTS idx_ai_threat_detections_agent_id ON public.ai_threat_detections(agent_id);
CREATE INDEX IF NOT EXISTS idx_ai_protection_rate_limits_user_endpoint ON public.ai_protection_rate_limits(user_id, endpoint);