...
import DownloadableCertificates from "@/components/DownloadableCertificates";
...
const AnalysisResults = ({ results, fileName, artworkId, userId, onTakeProtectionAction }: AnalysisResultsProps) => {
  ...
  // Check if any result has watermark detection
  const hasWatermarkDetection = results.some(result => result.watermarkDetected);
  
  // Get certificate data from results
  const certificateData = {
    fileName,
    artworkId,
    userId,
    watermarkId: results.find(r => r.watermarkDetected)?.watermarkId,
    confidence: results.find(r => r.watermarkDetected)?.confidence,
    scanType: results.find(r => r.watermarkDetected)?.type
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Downloadable Certificates Section */}
      <DownloadableCertificates
        hasWatermarkDetection={hasWatermarkDetection}
        hasBlockchainRegistration={false} // TODO: Add blockchain detection logic
