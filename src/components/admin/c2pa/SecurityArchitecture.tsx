...
const ARCHITECTURE_DOC = {
  ...
  product_version: '2.0',
  product_role: 'C2PA Generator',

  section_1_product_overview: {
    description: 'TSMO is a cloud-based content protection platform that applies C2PA Content Credentials to digital images (JPEG, PNG) and detects existing C2PA manifests in images and video (MP4, MOV). It operates as a Generator that creates, signs, and embeds C2PA manifests into user-uploaded content to establish verifiable provenance and assert AI training prohibitions.',
    supported_formats: ['image/jpeg', 'image/png'],
    validation_formats: ['image/jpeg', 'image/png', 'image/webp', 'video/mp4', 'video/quicktime'],
  ...
  section_2_target_of_evaluation: {
    components: [
      ...
      { name: 'sign-c2pa-manifest', type: 'Edge Function', description: 'Generates ES256 (ECDSA P-256) signatures over C2PA manifest claims using COSE Sign1 envelopes. Keys are stored in Supabase Secrets vault.' },
      { name: 'embed-c2pa-manifest', type: 'Edge Function', description: 'Constructs ISO 19566-5 JUMBF superboxes and writes them into JPEG (APP11 marker) or PNG (caBX chunk) files.' },
      { name: 'validate-c2pa-manifest', type: 'Edge Function', description: 'Binary scanner that detects existing C2PA JUMBF boxes in JPEG (APP11), PNG (caBX), and MP4/MOV (ISO BMFF uuid box) files.' },
    ...
    boundary_diagram: 'Client (Browser) → HTTPS → Supabase Edge Functions → Supabase Secrets Vault / PostgreSQL',
  },
  ...
  section_4_manifest_construction: {
    ...
    superbox_structure: [
      ...
      'Content Box: CBOR-encoded claim',
  ...
  section_7_threat_model: {
    threats_addressed: [
      ...
      'Manifest tampering: mitigated by COSE Sign1 cryptographic binding',
      'Unauthorized signing: mitigated by JWT authentication requirement',
      'Replay attacks: mitigated by unique instance_id (UUID) and timestamp per manifest',
